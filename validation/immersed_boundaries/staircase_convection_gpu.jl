...
simulation = Simulation(model, Δt=1e-2, stop_iteration=3200) #stop_time = 10)
...
function progress(sim)
    ...
    msg = @sprintf("Iter: %d, time: %s, wall time: %s, max|b|: %.3f, max|c|: %.3f, max|u|: %.2e, max|v|: %.2e, max|w|: %.2e",
...
record(fig, prefix * ".mp4", 1:Nt, framerate=30) do nn
